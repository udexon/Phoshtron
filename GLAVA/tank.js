...
// can only call F after m is initialized?
// aliases / colon definition words
m.F(": over 1 pick: ;");
m.F(": dup2 over over ;");
m.F(": p_a A ; : p_b B ;")
m.F(": is_1 1 - iz: ;")
...
// add tank 1 with Phos ??
  ...
  var S = window.S;
	...
	if (S[0].get_pos) {
	  ...
	  P = S[0].buf_pos;	    
	  S[P].push( JSON.stringify( scene.children[4].position ) ); // saved pointer, not value, must stringify at instance!
	  // chilren[4] = box behind tank
	  // how to make list of children ??
  }

	if (S[0].get_children) {   // different flag, not affecting get_pos
	  // console.log( "  render "+ Date.now() );
...
for (i = 0; i < children.length; i++) {
  ...
  if (typeof C === "number")
      S[P].push( JSON.stringify( [ scene.children[C].position, scene.children[C].rotation ] ) );
  else if (C.length==4) {
      var NC = scene.children[ C[0] ].children[ C[1] ].children[ C[2] ].children[ C[3] ];
      S[P].push( JSON.stringify( [ NC.position, NC.rotation ] ) );
      
  }
  else {
      
      var NC = scene.children[ C[0] ].children[ C[1] ].children[ C[2] ];
      S[P].push( JSON.stringify( [ NC.position, NC.rotation ] ) );
	  ...
	  // S[P].push( JSON.stringify( [ scene.children[4].position, scene.children[4].rotation ] ) );
	  // S[P].push( JSON.stringify( [ scene.children[4].position, scene.children[4].rotation, scene.children[4].position, scene.children[4].quaternion ] ) ); // saved pointer, not value, must stringify at instance!
	  // chilren[4] = box behind tank
	  // how to make list of children ??
  }
